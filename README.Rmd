...
[![Travis-CI Build Status](https://travis-ci.org/briandconnelly/softermax.svg?branch=master)](https://travis-ci.org/briandconnelly/softermax)
...
At the moment, only XML data are supported, which requires SoftMax Pro version 5.4 or greater.
...
```{r Installing via CRAN, eval=FALSE}
install.packages("softermax")
```

### Installing the Development Version via GitHub                            
                                                                                
You can use [devtools](https://cran.r-project.org/package=devtools) to install the development version from GitHub.
To do so:

```{r Installing development version, eval=FALSE}
...
For this example, I'll use an included sample file named `crystal_violet_dilutions.xml`.
